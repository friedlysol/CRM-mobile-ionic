...
export class GeolocationService implements SyncInterface {
   ...
   */
  public getCurrentLocation(): Promise<Location | null> {
    if (this.staticService.location && this.staticService.location.time) {
      if (moment(this.staticService.location.time).diff(moment.utc(), 'minutes') <= 2) {
